...
{
  {
    ...
    CHECK_EQ(rng->size(), 1);
    CHECK_EQ(rng->at(0), range::subrange_t { 900, 100 });
    CHECK_EQ((*rng)[0], range::subrange_t { 900, 100 });
  }
  {
    auto rng = range::parse("bytes=0-", 1000);
    CHECK_EQ(rng->unit(), "bytes");
    CHECK_EQ(rng->size(), 1);
    CHECK_EQ(rng->at(0), range::subrange_t { 0, 1000 });
    CHECK_EQ((*rng)[0], range::subrange_t { 0, 1000 });
  }
  {
    auto rng = range::parse("bytes=0-99, 200-", 1000);
    CHECK_EQ(rng->unit(), "bytes");
    CHECK_EQ(rng->size(), 2);
    CHECK_EQ(rng->at(0), range::subrange_t { 0, 100 });
    CHECK_EQ(rng->at(1), range::subrange_t { 200, 800 });
    CHECK_EQ((*rng)[0], range::subrange_t { 0, 100 });
    CHECK_EQ((*rng)[1], range::subrange_t { 200, 800 });
  }
  {
  ...
  }
  {
    ...
    CHECK_EQ(rng->size(), 3);
    CHECK_EQ(rng->at(0), range::subrange_t { 0, 100 });
    CHECK_EQ(rng->at(1), range::subrange_t { 200, 200 });
    CHECK_EQ(rng->at(2), range::subrange_t { 700, 50 });
    CHECK_EQ((*rng)[0], range::subrange_t { 0, 100 });
    CHECK_EQ((*rng)[1], range::subrange_t { 200, 200 });
    CHECK_EQ((*rng)[2], range::subrange_t { 700, 50 });
  }
  {
    ...
    CHECK(!range::parse("bytes=0;-99", 1000));
    CHECK(!range::parse("bytes=0-;99", 1000));
    CHECK(!range::parse("bytes=99-0", 1000));
    CHECK(!range::parse("bytes=-", 1000));
    CHECK(!range::parse("bytes=--100", 1000));
