...
TEST_CASE("integral")
{
  ...
  {
    CHECK(!from_string<std::uint32_t>(""));
    CHECK(!from_string<std::uint32_t>("."));
    CHECK(!from_string<std::uint32_t>("1."));
    CHECK(!from_string<std::uint32_t>("1.0"));
    CHECK(!from_string<std::uint32_t>(".1"));
    CHECK(!from_string<std::uint32_t>("1.1"));
    CHECK(!from_string<std::uint32_t>("1;"));
  }
}

template <typename T>
bool approx(expected<T, std::error_code> lhs, T rhs)
{
...
{
  {
    ...
    CHECK(approx(from_string<float>(n), -123.456f));
    CHECK(approx(from_string<double>(n), -123.456));
    CHECK(approx(from_string<long double>(n), -123.456l));
  }
  {
    CHECK(approx(from_string<float>("1"), 1.0f));
    CHECK(approx(from_string<float>("1."), 1.0f));
    CHECK(approx(from_string<float>(".1"), 0.1f));
    CHECK(approx(from_string<float>("1.1"), 1.1f));
    CHECK(approx(from_string<float>("-1"), -1.0f));
    CHECK(approx(from_string<float>("-1."), -1.0f));
    CHECK(approx(from_string<float>("-.1"), -0.1f));
    CHECK(approx(from_string<float>("-1.1"), -1.1f));
    CHECK(!from_string<float>(""));
    CHECK(!from_string<float>("."));
    CHECK(!from_string<float>("-."));
    CHECK(!from_string<float>("1;"));
  }
}
